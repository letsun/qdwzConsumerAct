...
function getJsApSign2(reurl,wxappid,corpid,getJsUrl){
	$.ajax({
		type: 'GET',
		url:getJsUrl+"/wap/wechat/getJsSign/"+corpid,
		data:{
			reurl:reurl,
			ts:(new Date()).getTime()
		},
...
//微信分享
function wx_Share(reurl,shareData,isLoadSound){
	var getJsUrl='https://order.letsun.com.cn';
	var corpid = '203';
	var wxappid = "wx1ee613cd8270ac7c"; //订阅号

	if (reurl.indexOf("https://test-qdwzact.lxcyhd.com") >= 0){
		//用pop的参数
        corpid = '247';
